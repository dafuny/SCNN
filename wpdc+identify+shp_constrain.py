...
from sia_loss import WPDCLoss, WPDCLoss_re,WPDCLoss_0, WPDCLoss_1, WPDCLoss_2, WPDCLoss_3, WPDCLoss_4
...
def train(train_loader, model, criterion_wpdc, criterion_iden, criterion_shp, optimizer_wpdc, optimizer_iden, optimizer_shp, epoch):
    #status:training!
    model.train()
    ...
    for i, (img_l, img_r, label, target_l, target_r) in enumerate(train_loader):
        ...
        loss = loss_wpdc+loss_shp+loss_iden;

        #log
        if i % epochs == 0:
            print('[Step:%d | Epoch:%d], lr_iden:%.8f, lr_shp:%.8f, lr_wpdc:%.8f, loss_iden:%.8f, loss_shp:%.8f, loss_wpdc:%.8f, loss:%.8f' % (i, epoch, lr_iden, lr_shp, lr_wpdc, loss_iden.data.cpu().numpy(), loss_shp.data.cpu().numpy(), loss_wpdc.data.cpu().numpy(), loss.data.cpu().numpy()))
            print('[Step:%d | Epoch:%d], lr_iden:%.8f, lr_shp:%.8f, lr_wpdc:%.8f, loss_iden:%.8f, loss_shp:%.8f, loss_wpdc:%.8f, loss:%.8f' % (i, epoch, lr_iden, lr_shp, lr_wpdc, loss_iden.data.cpu().numpy(), loss_shp.data.cpu().numpy(), loss_wpdc.data.cpu().numpy(), loss.data.cpu().numpy()), file=open(log_file + 'contrastive_print.txt','a'))
...
#    criterion = WPDCLoss_re().cuda()
    ...
    criterion_iden = WPDCLoss_2().cuda()
    criterion_shp = WPDCLoss_4().cuda()
    ...
    optimizer_iden = torch.optim.SGD(model.parameters(),
                                ...
                                weight_decay = weight_decay,
                                nesterov = True)
    optimizer_shp = torch.optim.SGD(model.parameters(),
                                lr = base_lr_shp,
                                momentum = momentum,
                                weight_decay = weight_decay,
                                nesterov = True)
#    optimizer = torch.optim.Adam(model.parameters(),lr = base_lr)
    ...
    for epoch in range(start_epoch, epochs+1):
        ...
        adjust_lr_shp(optimizer_shp, base_lr_shp, epoch, epochs, 30)
        #train for one epoch
        train(train_loader, model, criterion_wpdc, criterion_iden, criterion_shp,
              optimizer_wpdc, optimizer_iden, optimizer_shp, epoch)
        #save model paramers
